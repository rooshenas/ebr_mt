...
echo 'downloading data+preprocess'
...
--nbest 100 --beam 100 --batch-size 64 --remove-bpe @@  | tee iwdeen.out


grep ^S iwdeen.out | cut -f2- > train-source.txt
grep ^T iwdeen.out | cut -f2- > train-target.txt
grep ^H iwdeen.out | cut -f3- > train-hypo.txt
...
grep ^S iwdeen.out | cut -f2- > val-source.txt
grep ^T iwdeen.out | cut -f2- > val-target.txt
grep ^H iwdeen.out | cut -f3- > val-hypo.txt
...
grep ^S iwdeen.out | cut -f2- > test-source.txt
grep ^T iwdeen.out | cut -f2- > test-target.txt
grep ^H iwdeen.out | cut -f3- > test-hypo.txt
