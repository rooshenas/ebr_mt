...
bert_tokenizer = BertTokenizer.from_pretrained('/users/sbhatta9/conditionedbert/multibert')
...
class BertLMModel(nn.Module) : 
    ...
    def __init__(self):
        super().__init__()
        self.bert_model = BertForMaskedLM.from_pretrained('/users/sbhatta9/output/wmtdeen/checkpoint-74500', output_hidden_states = True) 
...
def output(data_iter, model, field_names):    
    ...
    for i, batch in enumerate(data_iter):
        ...
        for it, idx in enumerate(max_idx):
            ...
            print('#######hypo_texts',hypo_texts)
            f=bert_tokenizer.convert_tokens_to_string(bert_tokenizer.convert_ids_to_tokens(hypo_texts[0],skip_special_tokens = False))    ## entire sentence 
            print('###hypo',f)
            fb= bert_tokenizer.decode(bert_tokenizer.encode(f.split('[SEP]')[1]),skip_special_tokens = True) ##### after sep sentence
            print('###hypob',fb)

        for i in range(len(target_texts.tolist())):
            f=bert_tokenizer.convert_tokens_to_string(bert_tokenizer.convert_ids_to_tokens(target_texts.tolist()[i],skip_special_tokens = False))   
            print('#########target',f)     
            fb= bert_tokenizer.decode(bert_tokenizer.encode(f.split('[SEP]')[1]),skip_special_tokens = True) 
            print('#####targetb',fb)
            
            fs=bert_tokenizer.decode(bert_tokenizer.encode(f.split('[SEP]')[0]),skip_special_tokens = True) 
            print('#########targets',fs)
            l.append(fb)
            b.append(fs)
...
def inference(args) :
    ...
    model = BertLMModel()
    model.to(device)
    ckpt=torch.load('/users/sbhatta9/checkpoints_newdatagpuNOMAP7days/model_56_39.13.pt', map_location=device)
